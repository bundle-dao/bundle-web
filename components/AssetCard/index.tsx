            ...
            <Card style={{ height: '40px' }}>
                <Row>
                    ...
                    <Col xs={0} md={5} lg={9} style={{alignItems: 'flex-end'}}>
                        <ProgressBar 
                            progress={Math.round(props.asset.amount?.mul(props.asset.price!).div(parseEther('1')).div('10000000000000').toNumber()! / props.nav.div('1000000000000000').toNumber()!)} 
                        />
                    </Col>
                    <Col xs={6} md={4} lg={4} style={{alignItems: 'center'}}>
                        <Text style={{color: '#E7694C'}}>{`${Math.round(props.asset.amount?.mul(props.asset.price!).div(parseEther('1')).div('100000000000').toNumber()! / props.nav.div('1000000000000000').toNumber()!) / 100}%`}</Text>
                    </Col>
                </Row>
            </Card>
        </Col>
